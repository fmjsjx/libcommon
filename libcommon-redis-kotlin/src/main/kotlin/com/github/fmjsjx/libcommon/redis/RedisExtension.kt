 ...
 * @since 3.1
 */
suspend fun <K, V> RedisAsyncCommands<K, V>.tryLock(
    key: K,
    value: V,
    timeout: Long,
    maxWait: Long,
 ...
 * @since 3.1
 */
fun <K, V> RedisAsyncCommands<K, V>.unlockAsync(key: K, value: V): CompletionStage<Boolean> =
 ...
 * @since 3.1
 */
suspend fun <K, V> RedisAsyncCommands<K, V>.unlock(key: K, value: V): Boolean = unlockAsync(key, value).await()
